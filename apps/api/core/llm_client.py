...
class IAClient:
    ...
    async def completar_tarea_pesada(
        ...
        try:
            ...
            log.info(
                "llamando_llm_pesado_exito", model=model, tokens_usados=response.usage.total_tokens
            )
            return content
    ...
    async def completar_tarea_liviana(
        ...
        if not endpoint_id or not api_key:
            raise LLMCompletionError(
                "Falta la configuración de RUNPOD_ENDPOINT_OCR o RUNPOD_API_KEY."
            )
        ...
        log.info(
            "completar_tarea_liviana_runpod_inicio", endpoint_id=endpoint_id, prompt_len=len(prompt)
        )
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                if response.status_code != 200:
                    raise LLMCompletionError(
                        f"RunPod devolvió código de error {response.status_code}: {response.text}"
                    )
                ...
                while time.time() - start_time < timeout_seconds:
                    ...
                    if status_response.status_code != 200:
                        raise LLMCompletionError(
                            f"Error al verificar estado en RunPod: {status_response.text}"
                        )
                    ...
                    log.debug(
                        "completar_tarea_liviana_runpod_polling",
                        job_id=job_id,
                    ...
                    if current_status == "COMPLETED":
                        ...
                        log.info(
                            "completar_tarea_liviana_runpod_exito",
                            job_id=job_id,
                            chars=len(texto_generado),
                        )
                        return texto_generado

                    elif current_status == "FAILED":
                        error_msg = status_data.get(
                            "error", "Error desconocido en el worker de RunPod."
                        )
                ...
                raise LLMCompletionError(
                    f"Tiempo de espera agotado ({timeout_seconds}s) en RunPod."
                )
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                while time.time() - start_time < timeout_seconds:
                    ...
                    if status_response.status_code != 200:
                        raise OCRProcessingError(
                            f"Error al verificar estado: {status_response.text}"
                        )
                    ...
                    if current_status == "COMPLETED":
                        ...
                        raise OCRProcessingError(
                            f"El procesamiento del OCR falló en RunPod: {error_msg}"
                        )

                    elif current_status == "CANCELLED":
                        raise OCRProcessingError(
                            "El procesamiento del OCR fue cancelado en RunPod."
                        )
                ...
                raise OCRProcessingError(
                    f"Tiempo de espera agotado ({timeout_seconds}s) esperando el OCR en RunPod."
                )
