...
class CompletionTestRequest(BaseModel):
    ...
    modelo_tipo: str = Field(
        default="pesado", description="Tipo de modelo: 'pesado' (Gemini) o 'liviano' (RunPod)."
    )
...
_DEPENDENCIAS_SEGURIDAD = [Depends(get_current_user_or_agent)]
...
async def test_embedding(body: EmbeddingTestRequest):
    ...
    try:
        ...
        return {"dimensiones": len(vector), "valores_preview": vector[:5], "valores": vector}
    except IAClientError as e:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
...
async def test_ocr(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Fallo al procesar el OCR: {str(e)}"
        ) from e
